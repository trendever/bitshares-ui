...
class WalletUnlockModal extends React.Component {
    ...
    componentDidMount() {
        ...
        dispatcher.register( dispatch => {
          if ( dispatch.type === 'background-unlock' ) {
            this.onPasswordEnter(null,dispatch.data)
          }
        })
    ...
    onPasswordEnter(e, pswd) {
        const {passwordLogin} = this.props;
        if(e) e.preventDefault();
        let password = passwordLogin ? this.refs.password_input.value : this.refs.password_input.value();
        const account = passwordLogin ? this.state.account && this.state.account.get("name") : null;
        this.setState({password_error: null});

        if(typeof pswd == 'string') password = pswd
